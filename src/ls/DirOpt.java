...
public class DirOpt implements Comparable<DirOpt> {
	...
	public static final Comparator<DirOpt> LEN_CMP = new Comparator<DirOpt>() {
		@Override
	...
	public static List<DirOpt> stringToDirs (String dirStr, boolean enabled, boolean recursive) {
		...
		while (t.hasMoreTokens()) {
			...
			if (d.isDirectory()) {
				list.add(new DirOpt(d.getAbsoluteFile(),enabled,recursive));
			}
		}
		return list;
	}
	
	public static String dirsToString (Collection<DirOpt> dirs, boolean enabled, boolean recursive) {
		StringBuilder dirSb = new StringBuilder();
		for (DirOpt dir : dirs) {
			if (dir.enabled == enabled && dir.recursive == recursive) {
				if (dirSb.length() > 0) {
